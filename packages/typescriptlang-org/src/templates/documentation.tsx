...
// Hooks
import { useTableOfContents, MarkdownHeadingTreeNode } from "./hooks/useTableOfContents"
import { useHandbookFrontmatter } from "./hooks/useHandbookFrontmatter"
...
const HandbookTemplate: React.FC<Props> = (props) => {
  ...
  const { deprecationURL, showSidebar, showExperimental, isHandbook, title, preamble, oneline } = useHandbookFrontmatter(post.frontmatter as any)

  const sidebarHeaders = post.headings?.filter(h => (h?.depth || 0) <= 3) || []
  const tocTree = useTableOfContents(sidebarHeaders as any)

  useTwoslashHighlight(contentRef)
  useResponsiveImages(contentRef)
  ...
  const slug = slugger()
  return (
    <Layout title={`${prefix} - ${title}`} description={oneline || ""} lang={props.pageContext.lang} skipToAnchor="#handbook-content">
      <section id="doc-layout" >
        ...
        <div id="handbook-content" role="article">
          ...
          <h1>{title}</h1>
          {preamble && <div className="preamble" dangerouslySetInnerHTML={{ __html: preamble }} />}
          <article>
            <div className="whitespace raised">
              <div ref={contentRef} className="markdown" dangerouslySetInnerHTML={{ __html: post.html! }} />
            </div>
            {showSidebar &&
              <aside className="handbook-toc">
                <nav className={deprecationURL ? "deprecated" : ""} aria-label="table of contents">
                  {<>
                    ...
                    <MarkdownHeadingTree tree={tocTree} className="handbook-on-this-page-section-list" slug={slug} />
                  </>
                  }
